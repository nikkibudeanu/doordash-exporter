...
class IconProcessor(
    ...
    private fun loadIcons(): List<Icon> {
        ...
        return themeDirs.flatMap { dir ->
            ...
            val iconDir = File(dir.path.replace(".", "-")) /* Prism edit */
            val icons = iconDir.walk().filter { !it.isDirectory }.toList() /* Prism edit */
            println("Processing ${dir.name} icons under ${iconDir.absolutePath}")
            println("Number of icons to process: ${icons.size}")
