...
class IconTestingManifestGenerator(private val icons: List<Icon>) {
    ...
    fun generateTo(outputSrcDirectory: File) {
        ...
        icons
            .groupBy { it.theme }
            .map { (theme, icons) ->
                val propertyName = "${theme.themeClassName.replace(".", "")}Icons"
            ...
            .forEach { (theme, fileSpec) ->
                ...
                val wildcardImport =
                    "import ${PackageNames.PrismIconsPackage.packageName}.${theme.themePackageName}.*"
        ...
        FileSpec.builder(PackageNames.PrismDemoPackage.packageName, "AllIcons")
            .addProperty(
...
private fun generateListOfIconsForTheme(
    ...
    return FileSpec.builder(PackageNames.PrismDemoPackage.packageName, propertyName)
        .addAnnotation(
            AnnotationSpec.builder(Suppress::class)
                .addMember("%S", "NoWildcardImports")
                .addMember("%S", "WildcardImport")
                .build()
        )
        .addProperty(
...
private fun List<Icon>.toStatements(): List<CodeBlock> {
    return mapIndexed { index, icon ->
        buildCodeBlock {
            ...
            val text = "$iconFunctionReference to \"${ClassNames.Icons.simpleName}.${icon.theme.themeClassName}.${icon.kotlinName}\""
